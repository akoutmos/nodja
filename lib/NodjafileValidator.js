   ...
   */
    ...
    if (rules) {
        for (let rule in nodja_config.rules) {
            if (typeof nodja_config.rules[rule] !== 'string') {
                throw new NodjaValidationError(`Rule with key \'${rule}\' must be a string`);
            }
        }
    }
    ...
    if (rule_sets) {
        for (let set in nodja_config.rule_sets) {
            ...
            for (let rule in nodja_config.rule_sets[set]) {
                if (typeof nodja_config.rule_sets[set][rule] !== 'string') {
                    throw new NodjaValidationError(`Rule with key \'${rule}\' in rule set '${set}' must be a string`);
                }
            }
        }
    }
